#ifndef MONTY_H
#define MONTY_H
#include <stdio.h>
#include <stdlib.h>
#include <sys/types.h>
#include <unistd.h>
#include <fcntl.h>
#include <string.h>
#include <ctype.h>
/**
 * struct stack_s - doubly linked list representation of a stack (or queue)
 * @n: integer
 * @prev: points to the previous element of the stack (or queue)
 * @next: points to the next element of the stack (or queue)
 *
 * Description: doubly linked list node structure
 * for stack, queues, LIFO, FIFO Holberton project
 */
typedef struct stack_s
{
	int n;
	struct stack_s *prev;
	struct stack_s *next;
} stack_t;
/**
 * struct bus_s - bus_s
 * @arg: arg
 * @file: file
 * @content: content
 * @lifi: lifi
 * Description: Description
 */
typedef struct bus_s
{
	char *arg;
	FILE *file;
	char *content;
	int lifi;
}  bus_t;
extern bus_t bus;
/**
 * struct instruction_s - opcode and its function
 * @opcode: the opcode
 * @f: function to handle the opcode
 *
 * Description: opcode and its function
 * for stack, queues, LIFO, FIFO Holberton project
 */
typedef struct instruction_s
{
	char *opcode;
	void (*f)(stack_t **stack, unsigned int line_number);
} instruction_t;
char *_realloc(char *ptr, unsigned int old_size, unsigned int new_size);
ssize_t getstdin(char **lineptr, int file);
char  *clean_line(char *content);

void push(stack_t **sHead, unsigned int number);
void pall(stack_t **sHead, unsigned int number);
void pint(stack_t **sHead, unsigned int number);
int execute(char *content, stack_t **sHead, unsigned int lNumber, FILE *file);
void free_stack(stack_t *sHead);

void pop(stack_t **sHead, unsigned int lNumber);
void swap(stack_t **sHead, unsigned int lNumber);
void add(stack_t **sHead, unsigned int lNumber);
void nop(stack_t **sHead, unsigned int lNumber);
void sub(stack_t **sHead, unsigned int lNumber);
void divFunc(stack_t **sHead, unsigned int lNumber);

void mul(stack_t **sHead, unsigned int lNumber);
void mod(stack_t **sHead, unsigned int lNumber);
void pchar(stack_t **sHead, unsigned int lNumber);
void pstr(stack_t **sHead, unsigned int lNumber);

void rotl(stack_t **sHead, unsigned int lNumber);
void rotr(stack_t **sHead, __attribute__((unused)) unsigned int lNumber);

void addnode(stack_t **sHead, int n);
void addqueue(stack_t **sHead, int n);
void queue(stack_t **sHead, unsigned int lNumber);
void stack(stack_t **sHead, unsigned int lNumber);
#endif
